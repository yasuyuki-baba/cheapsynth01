...
void CS01LookAndFeel::drawLinearSlider(juce::Graphics& g, int x, int y, int width, int height,
    ...
    juce::Rectangle<float> thumb(track.getX(), sliderPos - thumbHeight / 2, trackWidth,
                                 thumbHeight);
...
void CS01LookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height,
    ...
    // Knob line
    juce::Path p;
    p.addCentredArc(bounds.getCentreX(), bounds.getCentreY(), arcRadius, arcRadius, 0.0f,
    ...
    juce::Point<float> thumbPoint(
        bounds.getCentreX() +
            (radius - thumbWidth * 0.5f) * std::cos(toAngle - juce::MathConstants<float>::halfPi),
        bounds.getCentreY() +
            (radius - thumbWidth * 0.5f) * std::sin(toAngle - juce::MathConstants<float>::halfPi));
