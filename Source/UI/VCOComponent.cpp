...
void VCOComponent::resized() {
    ...
    auto feetButtonArea =
        juce::Rectangle<int>(waveButtonArea.getRight(),  // Right edge of waveform button area
                             bounds.getY(),              // Start position of top area
    ...
    for (int i = 0; i < numWaveButtons; ++i) {
        ...
        auto buttonBounds = waveButtonArea.withHeight(waveButtonHeight)
                                .withY(waveButtonArea.getY() + i * waveButtonHeight);
    ...
    for (int i = 0; i < numFeetButtons; ++i) {
        ...
        auto buttonBounds = feetButtonArea.withHeight(feetButtonHeight)
                                .withY(feetButtonArea.getY() + i * feetButtonHeight);
        button->setBounds(buttonBounds.reduced(2));
    }
